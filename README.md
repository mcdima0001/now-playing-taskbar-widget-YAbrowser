# Now Playing Taskbar Widget — сборка с мостом к браузеру

Форк [Priler/now-playing-taskbar-widget](https://github.com/Priler/now-playing-taskbar-widget).
Добавлено то, ради чего форк и делался: виджет показывает трек из браузеров,
которые не сообщают о воспроизведении самой Windows. В первую очередь это
Яндекс.Браузер — он проигрывает звук собственным `YandexMediaPlayer` мимо
механизма Chromium, поэтому системная медиасессия у него не появляется вообще,
и обычный виджет его не видит.

## Установка

1. Скачайте `NowPlayingWidget-YAbrowser-Setup.exe` и запустите. Прав
   администратора не нужно, установка идёт в профиль пользователя. Если в
   системе нет .NET 8 Desktop Runtime, установщик скачает его сам.
2. Поставьте расширение для браузера — без него будут видны только приложения,
   которые сообщают о треке самой Windows (Spotify, Телеграм и подобные):
   - откройте `browser://extensions` (в Chrome и Edge — `chrome://extensions`);
   - включите **Режим разработчика**;
   - нажмите **Загрузить распакованное расширение** и выберите папку
     `bridge-extension` внутри папки установки (установщик предложит её открыть).

Расширение ставится распакованным, потому что в каталоге Chrome его нет.
Из-за этого браузер после установки может показывать предупреждение о
расширениях в режиме разработчика — это нормально.

## Что умеет сверх оригинала

- Трек, обложка, позиция и управление из вкладки браузера: Яндекс Музыка,
  YouTube, Spotify Web, SoundCloud.
- Клик по названию открывает то, откуда идёт звук: вкладку браузера или окно
  приложения.
- Пометка о ненормативной лексике рядом с названием (Яндекс Музыка).
- Избранное для трека из браузера.
- Привязка к содержимому кнопки погоды на панели задач, плавная смена ширины и
  плавная полоса прогресса.

Технические подробности и грабли — в [CLAUDE.md](CLAUDE.md).
